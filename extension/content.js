...
(async function () {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const claimantId = data.fields?.googleUserId?.stringValue || '';
            const claimantEmail = data.fields?.googleEmail?.stringValue || '';

            // Change button to "Taken by X" (Conditional color).
            setButtonTaken(addedBy, claimantId, claimantEmail);
    ...
    /**
     * Updates the UI to show the job is already claimed.
     * @param {string} userName - Name of the person who claimed the job.
     * @param {string} claimantId - Google User ID of the claimant.
     * @param {string} claimantEmail - Google Email of the claimant.
     */
    async function setButtonTaken(userName, claimantId, claimantEmail) {
        // Fetch current user info to compare
        let currentUserInfo = null;
        try {
            currentUserInfo = await new Promise((resolve) => {
                chrome.runtime.sendMessage({ action: 'getUserInfo' }, resolve);
            });
        } catch (e) {
            console.log('Could not get current user info for styling:', e);
        }

        const isMe = currentUserInfo &&
            claimantId === currentUserInfo.googleUserId &&
            claimantEmail === currentUserInfo.googleEmail;
        ...
        if (isMe) {
            // Light yellow box for the current user
            button.style.backgroundColor = '#fefcbf';
            button.style.color = '#744210'; // Dark brown/yellow text
            button.style.border = '1px solid #faf089';
        } else {
            // Standard red box for others
            ...
            chrome.storage.sync.get(['addedBy'], async (result) => {
                ...
                try {
                    ...
                    if (saveResponse.ok) {
                        ...
                        setButtonTaken(userName, googleUserId, googleEmail);
                        ...
                        const tId = data.fields?.googleUserId?.stringValue || '';
                        const tEmail = data.fields?.googleEmail?.stringValue || '';
                        setButtonTaken(takenBy, tId, tEmail);
